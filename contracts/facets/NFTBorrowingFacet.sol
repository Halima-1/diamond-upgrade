...
contract NFTBorrowingFacet {
    ...
    error NFTNotAvailableForBorrow();
    ...
    function borrowNFT(uint256 _tokenId, uint256 _duration) external {
        ...
        if (!info.active) revert NFTNotAvailableForBorrow();
        if (s.borrowings[_tokenId].active) revert NFTAlreadyBorrowed();
    ...
    function returnNFT(uint256 _tokenId) external {
        ...
        if (bInfo.borrower != msg.sender) revert NotBorrower();
        if (!bInfo.active) revert NoActiveBorrow();
